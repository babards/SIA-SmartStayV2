...
@section('content')
    <div class="container">
        ...
        <form method="GET" action="{{ route('tenant.properties.index') }}" class="mb-4">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-3">
                    <select name="landlord_filter" class="form-select" onchange="this.form.submit()">
                        ...
                        @foreach($properties->pluck('landlord')->unique('id') as $landlord)
                            @if($landlord)
                ...
                <div class="col-md-1">
                    <a href="{{ route('tenant.properties.index') }}" class="btn btn-outline-secondary w-100">Reset</a>
                </div>
            </div>
        </form>
            ...
            @foreach($properties as $property)
                <div class="col-md-3 mb-4">
                    <div class="card h-100 shadow-sm">
                        <a href="{{ route('tenant.properties.show', $property->propertyID) }}" style="text-decoration: none; color: inherit;">
                            @if($property->main_image)
                                <img src="{{ asset('storage/' . $property->main_image) }}" class="card-img-top property-img"
                                    alt="{{ $property->propertyName }}">
                            @else
                                <img src="https://via.placeholder.com/300x200?text=No+Image" class="card-img-top property-img"
                                    alt="No Image">
                            @endif
                            <div class="card-body">
        ...
        <!-- Map Section -->
        <div class="mt-5">
            <h3 class="text-center mb-4">Available Properties Map</h3>
...
document.addEventListener('DOMContentLoaded', function() {
                ...
                const popupContent{{ $property->propertyID }} = `
                    <div style="min-width: 380px; max-width: 420px; padding: 5px;">
                        ...
                        <div style="margin-top: 6px; text-align: center;">
                            <a href="{{ route('tenant.properties.show', $property->propertyID) }}" class="btn btn-outline-secondary btn-sm" target="_blank" style="border: 1px solid #6c757d; color: #6c757d; background: transparent; text-decoration: none; padding: 4px 8px; border-radius: 4px; font-size: 12px;">
    ...
    function loadWeatherData(propertyId) {
        ...
        // Fetch weather data
        fetch(`/tenant/properties/${propertyId}/weather`)
